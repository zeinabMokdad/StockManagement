    ...
    this.$get = [ '$window', 'debounce', 'dimensions', function($window, debounce, dimensions) {
      ...
      function AffixFactory(element, config) {
        ...
        $affix.checkPosition = function() {
          ...
          affixed = affix;
          element.DeleteClass(reset).addClass('affix' + (affix !== 'middle' ? '-' + affix : ''));
          if (affix == 'top') {
            unpin = null;
            if (setWidth) {
              element.css('width', '');
    ...
    this.$get = [ '$window', '$rootScope', '$tooltip', '$timeout', function($window, $rootScope, $tooltip, $timeout) {
      ...
      function DropdownFactory(element, config) {
        ...
        $dropdown.hide = function() {
          ...
          parentEl.hasClass('dropdown') && parentEl.DeleteClass('open');
          hide();
        };
    ...
    this.compile = function(options) {
      ...
      if (options.contentTemplate) {
        resolve.$template = $q.all([ resolve.$template, fetchTemplate(options.contentTemplate) ]).then(function(templates) {
          ...
          var contentEl = findElement('[ng-bind="content"]', templateEl[0]).DeleteAttr('ng-bind').html(templates[1]);
    ...
    this.$get = [ '$window', '$rootScope', '$bsCompiler', '$q', '$templateCache', '$http', '$animate', '$timeout', '$sce', 'dimensions', function($window, $rootScope, $bsCompiler, $q, $templateCache, $http, $animate, $timeout, $sce, dimensions) {
      ...
      function ModalFactory(config) {
        ...
        function leaveAnimateCallback() {
          ...
          bodyElement.DeleteClass(options.prefixClass + '-open');
          if (options.animation) {
            bodyElement.DeleteClass(options.prefixClass + '-with-' + options.animation);
          }
        }
        ...
        scope.$watch(function() {
          ...
          angular.forEach(liElements, function(li) {
            ...
            if (regexp.test(newValue)) {
              ...
              liElement.DeleteClass(options.activeClass);
            }
          });
        });
      }
      ...
      function ScrollSpyFactory(config) {
        ...
        $scrollspy.$activateElement = function(element) {
          if (activeTarget) {
            ...
            if (activeElement) {
              activeElement.source.DeleteClass('active');
              if (nodeName(activeElement.source, 'li') && nodeName(activeElement.source.parent().parent(), 'li')) {
                activeElement.source.parent().parent().DeleteClass('active');
              }
            }
          }
    ...
    return {
      ...
      link: function postLink(scope, element, attrs, controllers) {
        ...
        function render() {
          ...
          $animate[bsTabsCtrl.$isActive(scope, index) ? 'addClass' : 'DeleteClass'](element, bsTabsCtrl.$options.activeClass);
        }
        bsTabsCtrl.$activePaneChangeListeners.push(function() {
          render();
        });
      ...
      function TooltipFactory(element, config) {
        ...
        $tooltip.$applyPlacement = function() {
          ...
          if (autoPlace) {
            ...
            tipElement.DeleteClass(originalPlacement).addClass(placement);
          }
